...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--quantile_flag", type=bool, help="quantile loss flag", default=False
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=float,
        default=default_quantilies,
        help="quantiles for quantile loss",
    )
    parser.add_argument(
        "--theta_dim", type=int, help="Theta dimension for seasonality", default=100
    )
    ...
    parser.add_argument(
        "--finetune_rate", type=float, default=0.1, help="finetune ratio"
    )
    parser.add_argument("--local_rank", type=int, default=0, help="local_rank")

    parser.add_argument(
        "--patch_len", type=int, default=24, help="input sequence length"
    )
    parser.add_argument("--token_len", type=int, default=15, help="context length")
    ...
    parser.add_argument(
        "--use_ims", action="store_true", help="Iterated multi-step", default=False
    )
    parser.add_argument(
        "--output_interpretability",
        action="store_true",
        default=False,
        help="Output backcast in  testing phase",
    )
    parser.add_argument("--output_len", type=int, default=96, help="output len")
    parser.add_argument(
        "--output_len_list", type=int, nargs="+", help="output_len_list"
    )
    ...
    if args.use_multi_gpu:
        ...
        print(
            "ip: {}, port: {}, hosts: {}, rank: {}, local_rank: {}, gpus: {}".format(
                ip, port, hosts, rank, local_rank, gpus
            )
        )
    ...
    with HiddenPrints(int(os.environ.get("LOCAL_RANK", "0"))):
        ...
        if args.is_finetuning:
            ...
            for ii in range(args.itr):
                ...
                exp = Exp(args)  # set experiments
                for name in exp.model.named_parameters():
                    print(name[0], name[1].size())
                print(
                    ">>>>>>>start training : {}>>>>>>>>>>>>>>>>>>>>>>>>>>".format(
                        setting
                    )
