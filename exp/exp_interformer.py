...
class Exp_Interformer_forecast(Exp_Basic):
    ...
    def _build_model(self):
        ...
        if not self.args.is_training:
            checkpoint_path = os.path.join(
                self.args.checkpoints, self.args.ckpt_path, "checkpoint.pth"
            )
    ...
    def _select_criterion(self):
        criterion = forecast_backcast_qt_loss(
            self.args.quantilies, self.args.quantile_flag
        )
        return criterion
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                ...
                if self.args.output_attention:
                    ...
                    outputs, _, _ = self.model(batch_x, None, None, None)[0]
                else:
                ...
                if self.args.use_ims:
                    ...
                    if flag == "vali":
                        ...
                        flag == "test"
                    ):  # in this case, only pred_len is used to calculate loss
                        pred = pred[:, -self.args.pred_len :, :]
                        true = true[:, -self.args.pred_len :, :]
                        loss = self.vali_loss_func(pred, true)
                else:
                    loss = self.vali_loss_func(
                        accurate_outputs[:, -self.args.pred_len :, :],
    ...
    def test(self, setting, test=0):
        ...
        attns = []
        folder_path = (
            "../../../data/Interformer/test_results/"
            + setting
            + "/"
            + self.args.data_path
            + "/"
            + f"{self.args.output_len}/"
        )
        print(folder_path)
        if (
            ...
            for output_ptr in range(len(self.args.output_len_list)):
                ...
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                    ...
                    for j in range(inference_steps):
                        if len(pred_y) != 0:
                            batch_x = torch.cat(
                                [batch_x[:, self.args.pred_len :, :], pred_y[-1]], dim=1
                            )
                            # tmp = batch_y_mark[:, j - 1:j, :]
                            # batch_x_mark = torch.cat([batch_x_mark[:, 1:, :], tmp], dim=1)

                        if self.args.output_attention:
                            outputs, attns, backcast, enc_out = self.model(
                        ...
                        f_dim = -1 if self.args.features == "MS" else 0
                        pred_y.append(
                            outputs[:, -self.args.pred_len :, pred_true_idx, :]
                        )
                    ...
                    if self.args.use_ims:
                        batch_y = batch_y[
                            :,
                            self.args.label_len : self.args.label_len + self.args.output_len,
                            :,
                        ].to(self.device)
                    else:
                    ...
                    if test_data.scale and self.args.inverse:
                        ...
                        backcast = test_data.inverse_transform(
                            backcast.squeeze(0)
                        ).reshape(shape)
                        enc_out = test_data.inverse_transform(
                            enc_out.squeeze(0)
                        ).reshape(shape)
                    ...
                    if i % 10 == 0:
                        ...
                        gt = true[0, 0:500, -1]
                        pd = pred[0, 0:500, -1]
                        bc = enc_out[0, 0:500, -1]
                        ip = input[0, 0:500, -1]
                        quantile_pred_list = [
                            quantile_pred[0, 0:500, -1]
                            for quantile_pred in quantile_pred_list
                        ]
                        if self.args.local_rank == 0:
                            if self.args.output_attention:
                                ...
                                attn_map(
                                    ...
                                    os.path.join(
                                        folder_path,
                                        f"attn_{i}_{self.args.local_rank}.pdf",
                                    ),
                                )
                            # Save output results, ground truth, and backcast into np files
                            np.save(
                                os.path.join(
                                    folder_path, f"pred_{i}_{self.args.local_rank}.npy"
                                ),
                                pred.detach().cpu().numpy(),
                            )
                            np.save(
                                os.path.join(
                                    folder_path, f"true_{i}_{self.args.local_rank}.npy"
                                ),
                                true.detach().cpu().numpy(),
                            )
                            np.save(
                                os.path.join(
                                    folder_path, f"backcast_{i}_{self.args.local_rank}.npy"
                                ),
                                backcast,
                            )
                            np.save(
                                os.path.join(
                                    folder_path, f"enc_out_{i}_{self.args.local_rank}.npy"
                                ),
                                enc_out,
                            )
                            np.save(
                                os.path.join(
                                    folder_path, f"input_{i}_{self.args.local_rank}.npy"
                                ),
                                input,
                            )
                            np.save(
                                os.path.join(
                                    folder_path,
                                    f"quantile_pred_{i}_{self.args.local_rank}.npy",
                                ),
                                np.array(quantile_pred_list),
                            )
                            # Save visualizations
                            ...
                            visual(
                                ...
                                pd,
                                os.path.join(
                                    folder_path, f"{i}_{self.args.local_rank}.pdf"
                                ),
                            )
                            ...
                            visual_multi(
                                ...
                                quantile_pred_list,
                                os.path.join(
                                    folder_path, f"multi_{i}_{self.args.local_rank}.pdf"
                                ),
                            )
                            ...
                            if self.args.output_interpretability:
                                visual(
                                    ...
                                    os.path.join(
                                        folder_path,
                                        f"backcast_{i}_{self.args.local_rank}.pdf",
                                    ),
                                )
                                '''
    ...
    def finetune(self, setting):
        ...
        for epoch in range(self.args.finetune_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(
                ...
                if self.args.use_ims:
                    # output used to calculate loss misaligned patch_len compared to input
                    loss, (forecast_loss, backcast_loss, quantile_loss) = criterion(
                        outputs[:, -self.args.seq_len :, :], batch_y, backcast, batch_x
                    )
                else:
                    # only use the forecast window to calculate loss
                    loss, (forecast_loss, backcast_loss, quantile_loss) = criterion(
                ...
                if i % 50 == 0:
                    ...
                    print(
                        "\titers: {0}, epoch: {1} | loss: {2:.7f} | forecast loss: {3:.7f}| backcast loss: {4:.7f}| quantile loss: {5:.7f}| cost_time: {6:.0f} | memory: allocated {7:.0f}MB, reserved {8:.0f}MB, cached {9:.0f}MB ".format(
                            i,
                            epoch + 1,
                            loss.item(),
                            forecast_loss.item(),
        ...
        seasonal_params = self.model.module.backbone.encoder.interpreter_layers[
            0
        ].state_dict()
        torch.save(seasonal_params, path + "/" + "seasonal_params.pth")
        trend_params = self.model.module.backbone.encoder.interpreter_layers[
            1
        ].state_dict()
